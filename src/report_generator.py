...
class ReportGenerator:
    ...
    def _script(self):
        return (
            ...
            "function loadPerf(){"
            # performance.json = calcule depuis data/predictions.csv (performance.py).
            "fetch('performance.json?t='+Date.now())"
            ".then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json();})"
            ".then(function(p){document.getElementById('perf-content').innerHTML=renderPerfJournal(p);})"
            ".catch(function(){document.getElementById('perf-content').innerHTML="
            "\"<div class='perf-empty'><div class='perf-empty-icon'>📊</div>"
            "<div class='perf-empty-title'>Aucune donnee de performance</div>"
            "<div class='perf-empty-sub'>docs/performance.json est produit a chaque Actualiser et par le workflow de fermeture.</div></div>\";});}"

            "function _pc(v,d){if(v===null||v===undefined)return '—';d=d===undefined?1:d;return (v>=0?'+':'')+(100*v).toFixed(d)+'%';}"
            "function _tile(lbl,val,sub){return \"<div class='perf-box'><div class='perf-label'>\"+lbl+\"</div><div class='perf-val' style='font-size:17px'>\"+val+\"</div><div class='trk-mini'>\"+sub+\"</div></div>\";}"

            # Courbe de calibration: une serie (pas de legende), diagonale de
            # reference recessive, points de 8px avec infobulle, table a cote.
            "function calSVG(cal){"
            "var W=250,H=180,L=34,B=26,T=8,R=16,w=W-L-R,hh=H-T-B;"
            "function X(v){return L+v*w;}function Y(v){return T+(1-v)*hh;}"
            "var s='<svg class=\"cal-svg\" viewBox=\"0 0 '+W+' '+H+'\" role=\"img\" aria-label=\"Courbe de calibration\">';"
            "[0,0.25,0.5,0.75,1].forEach(function(g){"
            "s+='<line x1=\"'+X(0)+'\" x2=\"'+X(1)+'\" y1=\"'+Y(g)+'\" y2=\"'+Y(g)+'\" class=\"cal-grid\"/>';"
            "s+='<text x=\"'+(L-4)+'\" y=\"'+(Y(g)+3)+'\" class=\"cal-ax\" text-anchor=\"end\">'+(g*100)+'%</text>';"
            "s+='<text x=\"'+X(g)+'\" y=\"'+(H-B+14)+'\" class=\"cal-ax\" text-anchor=\"middle\">'+(g*100)+'%</text>';});"
            "s+='<line x1=\"'+X(0)+'\" y1=\"'+Y(0)+'\" x2=\"'+X(1)+'\" y2=\"'+Y(1)+'\" class=\"cal-diag\"/>';"
            "var pts=cal.map(function(c){return [X(c.pred),Y(c.reel)];});"
            "if(pts.length>1)s+='<polyline points=\"'+pts.map(function(p){return p.join(',');}).join(' ')+'\" class=\"cal-line\"/>';"
            "cal.forEach(function(c,i){"
            "s+='<circle cx=\"'+pts[i][0]+'\" cy=\"'+pts[i][1]+'\" r=\"4\" class=\"cal-dot\"><title>Decile '+c.decile+' ('+Math.round(c.lo*100)+'-'+Math.round(c.hi*100)+'%): predit '+(c.pred*100).toFixed(1)+'%, reel '+(c.reel*100).toFixed(1)+'%, n='+c.n+'</title></circle>';"
            "s+='<circle cx=\"'+pts[i][0]+'\" cy=\"'+pts[i][1]+'\" r=\"10\" class=\"cal-hit\"><title>Decile '+c.decile+': predit '+(c.pred*100).toFixed(1)+'%, reel '+(c.reel*100).toFixed(1)+'%, n='+c.n+'</title></circle>';});"
            "s+='<text x=\"'+(L+w/2)+'\" y=\"'+(H-2)+'\" class=\"cal-ax\" text-anchor=\"middle\">probabilite predite</text>';"
            "return s+'</svg>';}"

            "function perfGroup(name,g){"
            "var c=g.clv||{},cr=g.clv_reference||{},r=g.roi||{},b=g.brier||{};"
            "var h=\"<div class='pj-group'><div class='perf-section-title'>\"+name+\" <span class='pj-n'>\"+g.n_predictions+\" predictions · \"+g.n_reglees+\" reglees</span></div><div class='perf-grid'>\";"
            ...
            "if(cal.length){h+=\"<div class='pj-cal'>\"+calSVG(cal)+\"<table class='pj-tbl'><thead><tr><th>Decile</th><th>n</th><th>Predit</th><th>Reel</th></tr></thead><tbody>\";"
            "cal.forEach(function(c){h+='<tr><td>'+Math.round(c.lo*100)+'-'+Math.round(c.hi*100)+'%</td><td>'+c.n+'</td><td>'+(c.pred*100).toFixed(1)+'%</td><td>'+(c.reel*100).toFixed(1)+'%</td></tr>';});"
            "h+='</tbody></table></div>';}"
            "return h+'</div>';}"

            "function renderPerfJournal(p){"
            "var ep=(p&&p.epoques)||{};var j=ep.journal||{},hi=ep.historique||{};"
            "var h=\"<div class='perf-title'>Performance — CLV, ROI, Brier, calibration</div>\";"
            ...
            "kh.forEach(function(k){h+=perfGroup(k,gh[k]);});h+='</details>';}"
            "return h;}"
    ...
    def _css(self):
        return (
            ...
            ".pj-group{margin:14px 0 18px}.pj-n,.pj-sub{font-weight:400;color:var(--m);font-size:11px;text-transform:none}"
            ".pj-cal{display:flex;gap:16px;align-items:flex-start;flex-wrap:wrap;margin-top:8px}"
            ".cal-svg{width:260px;max-width:100%;height:auto}"
            ".cal-grid{stroke:var(--b);stroke-width:1}.cal-diag{stroke:var(--m);stroke-width:1;stroke-dasharray:4 3}"
            ".cal-line{fill:none;stroke:#2563EB;stroke-width:2}.cal-dot{fill:#2563EB;stroke:var(--s);stroke-width:2}"
            ".cal-hit{fill:transparent;cursor:default}.cal-ax{font-size:9px;fill:var(--m)}"
            ".pj-tbl{font-size:11px;border-collapse:collapse;width:auto}.pj-tbl th,.pj-tbl td{padding:2px 8px;text-align:right;color:var(--t)}"
            ".pj-tbl th{color:var(--m);font-weight:500}"
            ".pj-hist{margin-top:18px}.pj-hist summary{cursor:pointer;color:var(--m);font-size:12px}"
