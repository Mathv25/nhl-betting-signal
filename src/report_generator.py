...
class ReportGenerator:
    ...
    def _js_render(self):
        ...
        return (
            ...
            "btn.disabled=true;"
            "try{"
            "var r=await fetch('signal.json?t='+Date.now());"
            "if(!r.ok)throw new Error('HTTP '+r.status);"
            "gSignal=await r.json();"
            "renderAll(gSignal);"
            "btn.textContent='\u2713 Actualise';"
            "setTimeout(function(){btn.textContent=orig;btn.disabled=false;},2500);"
            "}catch(e){"
            "alert('Erreur actualisation: '+e.message);"
            "btn.textContent=orig;btn.disabled=false;}}"

            "function renderAll(d){"
            "renderHeaderGrid(d);renderSignalTab(d);renderPropsTab(d);renderNBATab(d);renderMLBTab(d);}"
