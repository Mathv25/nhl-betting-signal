...
class ReportGenerator:
    ...
    def _nav(self):
        return (
            ...
            "</div>"
            "<button id=\"refreshBtn\" onclick=\"manualRefresh()\" title=\"Verifier nouveau signal\" style=\""
    ...
    def _script(self):
        return (
            ...
            "btn.textContent='↻ ...';"
            "btn.disabled=true;"
            "fetch('signal.json?t='+Date.now())"
            ".then(function(r){return r.json();})"
            ".then(function(d){"
            "var current=document.querySelector('meta[name=\"signal-generated-at\"]');"
            "var currentTs=current?current.getAttribute('content'):'';"
            "if(d.generated_at&&d.generated_at!==currentTs){"
            "btn.textContent='↻ Nouveau signal!';"
            "btn.style.color='var(--g)';"
            "btn.style.borderColor='var(--g)';"
            "btn.disabled=false;"
            "btn.onclick=function(){location.reload(true);};"
            "}else{"
            "btn.textContent='✓ A jour';"
            "btn.style.color='var(--m)';"
            "setTimeout(function(){btn.textContent='↻ Actualiser';btn.disabled=false;btn.onclick=manualRefresh;},2000);"
            "}})"
            ".catch(function(){"
